...
import { APBill, APBillStatus, PaymentMethod } from '../../types';
...
export const AccountsPayableView: React.FC = () => {
  ...
  const [activeSubTab, setActiveSubTab] = useState<'bills' | 'schedule' | 'disbursements'>('bills');
  ...
  const [payingBill, setPayingBill] = useState<APBill | null>(null);
  ...
  const scheduleBuckets = useMemo(() => {
    const buckets: Record<string, { count: number; total: number; bills: APBill[] }> = {
  ...
  const handleOpenPay = (bill: APBill) => {
    setPayingBill(bill);
