          ...
          <table id="jobs-table" className="w-full text-left text-xs text-stone-700">
            ...
            <tbody className="divide-y divide-stone-100">
              {paginatedJobs.map(j => {
                ...
                return (
                  <tr key={j.id} id={`job-row-${j.id}`} className="hover:bg-stone-50/60 transition-colors">
                    <td className="px-4 py-3 font-mono font-medium text-stone-900 whitespace-nowrap">
                      {j.id}
                    </td>
                    <td className="px-4 py-3">
                      <div className="font-semibold text-stone-900">{j.title}</div>
                      <div className="text-[11px] text-stone-400 truncate max-w-sm">
                        {j.description || `${j.employmentType} position`}
                      </div>
                    </td>
                    <td className="px-4 py-3 font-medium text-stone-800">
                      {client?.name || j.clientId}
                    </td>
                    <td className="px-4 py-3 text-stone-600">{j.department}</td>
