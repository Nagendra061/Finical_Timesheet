...
import { CSVUploader } from '../imports/CSVUploader';

/**
 * ImportsView Component
 * Renders CSV import validation, dry-run schema checking, and batch ingestion.
 *
...
export const ImportsView = () => {
  ...
  const [csvRawContent, setCsvRawContent] = useState(SAMPLE_CSV_TEMPLATES.employees || '');
  const [fileName, setFileName] = useState('employees-sample.csv');
  ...
  const handleExecuteImport = () => {
    ...
    setTimeout(() => {
      const batch = processCSVImport(entityType, csvRawContent, fileName);
      ...
      <div className="bg-white rounded-xl border border-stone-200 p-5 shadow-xs space-y-4">
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
            {['employees', 'clients', 'placements', 'timesheets', 'payments'].map(type => (
              <button
                key={type}
                type="button"
        ...
        <CSVUploader
          ...
          fileName={fileName}
          csvRawContent={csvRawContent}
          onContentChange={val => {
            setCsvRawContent(val);
            setValidationResult(null);
          }}
              ...
              <div className="border border-stone-200 rounded-lg overflow-hidden max-h-48 overflow-y-auto">
                <table className="w-full text-left text-xs">
                  ...
                  <tbody className="divide-y divide-stone-100">
                    {validationResult.validRows.map((vr) => (
                      <tr key={vr.rowIndex}>
