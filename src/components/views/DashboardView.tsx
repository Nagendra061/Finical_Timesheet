...
export const DashboardView: React.FC<DashboardViewProps> = ({ onNavigate }) => {
  const { employees, clients, placements, timesheets, incomes, invoices, apBills } = useStaffing();
  ...
  const apAgingCounts: Record<string, { count: number; total: number }> = {
    Current: { count: 0, total: 0 },
  ...
  incomes.forEach(inc => {
    ...
    revenueByClientMap[name] = (revenueByClientMap[name] || 0) + inc.totalIncome;
  });
  ...
  return (
    <div className="space-y-6 pb-12 animate-fade-in">
      ...
      {pendingTimesheets.length > 0 && (
        <div className="rounded-xl p-4 bg-amber-50 border border-amber-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-xs">
          ...
          <button
            ...
            className="px-3.5 py-1.5 rounded-lg bg-amber-600 hover:bg-amber-700 text-white font-medium text-xs self-start sm:self-center transition-colors flex items-center gap-1.5"
          >
            Review Timesheets
      ...
      {/* 9 KPI Cards as explicitly specified */}
      <div>
          ...
          <div>
            ...
            <div className="mt-4 space-y-2">
              <button
                ...
                type="button"
                onClick={() => onNavigate('timesheets')}
                className="w-full flex items-center justify-between p-2.5 rounded-lg border border-stone-200 hover:bg-stone-50 text-left text-xs transition-colors"
              >
