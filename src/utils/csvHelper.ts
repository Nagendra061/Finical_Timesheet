...
import { ImportFileType, ImportError } from '../types';
...
export function validateCSVRows(
  ...
  const rejectedRecords: { row: number; data: Record<string, string>; errors: string[] }[] = [];
  ...
  rows.forEach((row, index) => {
    ...
    switch (fileType) {
      ...
      case 'timesheets': {
        const id = row.timesheet_id || row.id;
...
export function validateCSVData(
  ...
  const validRows: { rowIndex: number; data: Record<string, string> }[] = [];
  const rejectedRows: { rowIndex: number; rawRow: Record<string, string>; errors: string[] }[] = [];
